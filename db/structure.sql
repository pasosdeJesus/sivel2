...
CREATE VIEW public.cvt1 AS
 SELECT DISTINCT acto.caso_id,
   ...
   FROM ((((((((public.sivel2_gen_acto acto
     ...
     LEFT JOIN public.msip_ubicacion ubicacion ON ((caso.ubicacion_id = ubicacion.id)))
     LEFT JOIN public.msip_departamento departamento ON ((ubicacion.departamento_id = departamento.id)))
     LEFT JOIN public.sivel2_gen_presponsable presponsable ON ((acto.presponsable_id = presponsable.id)))
  WHERE ((categoria.id = ANY (ARRAY[20, 10, 30])) AND (categoria.id = ANY (ARRAY[25, 15, 35, 55, 45, 73, 65, 92, 50, 40, 67, 801, 90, 37, 46, 57, 16, 26, 80, 85, 66, 64, 703, 18, 706, 59, 49, 38, 28, 501, 401, 135, 115, 125, 904, 402, 17, 231, 502, 331, 705, 62, 503, 906, 104, 403, 713, 101, 76, 11, 21, 302, 27, 34, 902, 102, 903, 14, 301, 24, 30, 10, 20, 63, 93, 910, 714, 78, 89, 905, 86, 701, 68, 241, 341, 141, 715, 704, 702, 53, 43, 13, 23, 33, 88, 98, 84, 709, 711, 707, 708, 710, 87, 97, 717, 917, 716, 916, 91, 95, 718, 48, 58, 75, 69, 41, 74, 72, 12, 36, 22, 47, 56, 520, 19, 420, 39, 77, 29, 712])));
